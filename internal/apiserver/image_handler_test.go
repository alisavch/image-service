	...
	tests := []struct {
		...
		},
		{
			...
			fn: func(mockSO *mocks.ServiceOperations, token string, compressedID uuid.UUID, isOriginal bool) {
				...
				s := uuid.MustParse(asString)
				mockSO.On("ParseToken", token).Return(s, nil)
				mockSO.On("IsAuthenticated", mock.Anything, s, compressedID).Return(nil)
				mockSO.On("FindRequestStatus", mock.Anything, s, compressedID).Return(models.Status(""), utils.ErrUserAuthentication)
			},
			expectedStatusCode:   404,
